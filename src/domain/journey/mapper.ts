...
export function journeyToEditorState(journey: Journey): {
  ...
  for (const node of journey.nodes) {
    if (
      node.type === "STAGE" &&
      (node.data as { isCollapsed?: boolean }).isCollapsed
    ) {
  ...
  const nodes: EditorNode[] = journey.nodes.map((node) => {
    ...
    const isInsideCollapsed =
      node.parentId ? collapsedStageIds.has(node.parentId) : false;
    const isStageCollapsed =
      node.type === "STAGE" &&
      !!(node.data as { isCollapsed?: boolean }).isCollapsed;
    const nodeHeight = isStageCollapsed ? 56 : node.size.height;
    ...
    return {
      ...
      data: {
        ...
        expandedHeight: node.size.height,
      },
      style: {
...
export function editorStateToJourney(
  ...
  const domainNodes: JourneyNode[] = nodes.map((flowNode) => {
    ...
    if (
      domainType === "STAGE" &&
      (flowNode.data as { isCollapsed?: boolean }).isCollapsed
    ) {
      height =
        (flowNode.data as { expandedHeight?: number }).expandedHeight ||
        defaultSize.height;
    }
  ...
  const domainEdges: JourneyEdge[] = edges.map((flowEdge) => {
    return {
      ...
      label:
        flowEdge.data?.label ||
        (typeof flowEdge.label === "string" ? flowEdge.label : null),
